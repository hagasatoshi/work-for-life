class CommitsController < ApplicationController
  ...
  def set_orgs
    ...
    redirect_with_error_message '参照が許可されているGitHub Organizationが存在しないため実行できません' unless @orgs.present?
  end

  def set_repos
    @repos = @retrieve.repositories(index_params[:org_id].to_i)
    redirect_with_error_message '参照が許可されているGitHub Organizationが存在しないため実行できません' unless @repos.present?
  end
